  ...
  .directive('ecVcms', ['$timeout', ($timeout) => ({
    ...
    controller: [
      '$scope', '$element', '$attrs', '$transclude', function (scope, elem, attrs, transclude) {
        ...
        self.colors = scope.colors || ['red', 'yellow', 'green', 'blue'];
        ...
        $timeout(() => {
          transclude(scope, (clone, scope) => {
            if (editor.innerHTML !== '') {
              scope.json = vcms.toJSON(editor.innerHTML.replace(/contenteditable="(\w*)"/ig, ''));
            }
        ...
        function refresh() {
          ...
          $timeout(() => {
            scope.json = editor.innerHTML === '' ? [] : vcms.toJSON(editor.innerHTML.replace(/contenteditable="(\w*)"/ig, ''));
            scope.html = vcms.toDOM(scope.json);
        ...
        self.toggleHtml = function (e) {
          $timeout(() => {
            ...
            if (!codemirror) {
              ...
              codemirror.setValue(editor.innerHTML || '');
              $timeout(() => {
                codemirror.refresh();
              });
            }
        ...
        editor.addEventListener('mousedown', (e) => {
          if (self.currentElement && self.currentElement[0] !== e.target) {
