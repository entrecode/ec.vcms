  ...
  .directive('ecVcms', ['$timeout', $timeout => ({
    ...
    controller: [
      '$scope', '$element', '$attrs', '$transclude', function (scope, elem, attrs, transclude) {
        ...
        scope.$watch('json', () => {
          if(editor.innerHTML.replace(/\s?contenteditable="(\w*)"/ig, '') !== vcms.toDOM(scope.json)) {
            editor.innerHTML = vcms.toDOM(scope.json);
          }
        });
        ...
        function prepareHTML(html) {
          return html
          .replace(/\s?contenteditable="(\w*)"/ig, '')
          .replace(/&nbsp;/g, '\u00a0')
          .replace(/<br>/g, '<br />');
        }
        ...
        function refresh() {
          ...
          $timeout(() => {
            scope.json = editor.innerHTML === '' ? [] : vcms.toJSON(prepareHTML(editor.innerHTML));
            scope.html = vcms.toDOM(scope.json);
            if (scope.vcms.currentElement) {
              scope.vcms.tag = scope.vcms.currentElement[0].localName;
        ...
        scope.$watch('json', () => {
          if (prepareHTML(editor.innerHTML) !== vcms.toDOM(scope.json)) {
            editor.innerHTML = vcms.toDOM(scope.json);
          }
        });
        ...
        scope.vcms.beautifyHtml = function (e) {
          editor.innerHTML = pretty(editor.innerHTML, { ocd: true });
          codemirror.setValue(pretty(prepareHTML(editor.innerHTML), { ocd: true }));
          prevent(e);
        };
          ...
          $timeout(() => {
            ...
            if (!codemirror) {
              codemirror = CodeMirror(elem[0].querySelector('.vcms-html'), {
                value: prepareHTML(editor.innerHTML),
                lineNumbers: true,
                lineWrapping: true,
                mode: 'htmlmixed',
              });
              ...
              codemirror.setValue(prepareHTML(editor.innerHTML));
              $timeout(() => {
                codemirror.refresh();
              });
            }
